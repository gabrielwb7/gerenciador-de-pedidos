...
public class TestConfig implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Order orderOne = Order.builder().moment(Instant.parse("2019-06-20T19:53:07Z")).orderStatus(OrderStatus.DELIVERED).client(user).build();
        Order orderTwo = Order.builder().moment(Instant.parse("2022-02-20T18:33:44Z")).orderStatus(OrderStatus.CANCELED).client(userTwo).build();
        Order orderThree = Order.builder().moment(Instant.parse("2021-04-20T20:54:27Z")).orderStatus(OrderStatus.WAITING_PAYMENT).client(user).build();
